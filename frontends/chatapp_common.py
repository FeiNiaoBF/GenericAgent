...
# ========== lifecycle notification utilities ==========
import os as _os, threading as _threading, time as _time

def _get_notify_chat_id():
    return _os.environ.get("GA_NOTIFY_CHAT_ID", "")

def _get_shutdown_file():
    return _os.environ.get("GA_SHUTDOWN_FILE", "")

def setup_lifecycle_monitor(label, send_startup, send_shutdown, on_shutdown_requested=None):
    """Setup lifecycle: send startup notification, then monitor shutdown signal.
    
    Returns shutdown_flag (threading.Event) for main loop to check.
    Caller should poll shutdown_flag.is_set() to exit gracefully.
    """
    shutdown_flag = _threading.Event()
    notify_id = _get_notify_chat_id()
    
    # Send startup notification
    if notify_id and send_startup:
        try:
            send_startup(notify_id, label)
        except Exception as e:
            print(f"[{label}] Startup notification failed: {e}")
    
    # Monitor shutdown signal file
    shutdown_file = _get_shutdown_file()
    if not shutdown_file:
        print(f"[{label}] No shutdown file configured, graceful shutdown disabled")
        return shutdown_flag
    
    def _monitor():
        while not shutdown_flag.is_set():
            if _os.path.exists(shutdown_file):
                print(f"[{label}] Shutdown signal received: {shutdown_file}")
                if send_shutdown and notify_id:
                    try:
                        send_shutdown(notify_id, label)
                    except Exception as e:
                        print(f"[{label}] Shutdown notification failed: {e}")
                try:
                    _os.remove(shutdown_file)
                except:
                    pass
                if on_shutdown_requested:
                    try:
                        ...
                        print(f"[{label}] Shutdown callback failed: {e}")
                shutdown_flag.set()
                return
            _time.sleep(2)
