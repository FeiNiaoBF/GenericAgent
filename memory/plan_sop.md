...
### 零步：复杂度评估与路由（来自KARIMO）

进入plan模式前快速评估任务复杂度，决定资源分配：

| 信号 | 简单 | 中等 | 复杂 |
|------|------|------|------|
| 步骤数 | 3-5步 | 6-10步 | 10+步 |
| 依赖关系 | 线性链 | 有分支 | 多层嵌套+循环 |
| 涉及文件 | <5个 | 5-20个 | 20+个或跨项目 |
| 是否有未知 | 全部已知 | 部分探索 | 大量探索 |
| **并行策略** | 无/简单 | 分组并行 | 波浪并行（见执行态） |

- **简单**：常规执行，无需特殊并行
- **中等**：分组独立步骤并行执行（每组≤2个subagent）
- **复杂**：波浪并行（先执行无依赖任务，汇聚后再执行依赖任务），并发上限3个subagent

---

## 零. 快速检查模式（1-2步简单任务，跳过plan）

适用场景：改一行配置、读一个文件、装个包等极简任务

直接执行，无需创建plan.md。完成即可回复主人。
...
每步必须能用1-3次工具调用完成，否则拆分：

| 过粗（需拆） | 合适 | 过细（需合） |
|--------------|------|-------------|
| "重构整个模块" | "修改config.py的timeout字段" | "打开文件→读第1行→读第2行..." |
| "处理所有文件" | "修改5个目标文件的import语句" | "保存→确认保存成功" |
| 无法Mini验证 | 能快速验证产出 | 验证本身就是一步 |
...
### 波浪并行执行（来自planning-with-files，复杂任务用）

当步骤互相独立且耗时较长时，可组批并行：

1. **扫描plan.md**，收集所有无依赖的 `[ ]` 步骤
2. **分批启动** subagent 并行执行（每批≤3个），每个subagent负责一个独立步骤
3. **等待汇总**：所有subagent返回后，标记完成 `[✓]`，再扫描下一波
4. **有依赖的步骤**按顺序串行执行

⚡ **并行条件**：步骤间无数据依赖 + 各自SOP明确 + 不操作同一文件
⚡ **回退规则**：并行失败时，回退为串行逐个执行
...
2. **自动重试**：网络超时→重试3次(2s/4s/8s指数退避) | 文件锁冲突→等待5s重试 | 端口占用→换端口重试
3. **subagent失败**：查stderr.log→明确错误修正重启 | 未知错误重试1次 | 最多重启2次

### 第二阶：降级执行（重试3次仍失败）
1. **简化策略**：去除失败步骤的可选部分，只保留核心产出
2. **替代方案**：尝试用不同工具/方法达成同一目标（如API失败→浏览器抓取）
3. **部分跳过**：非关键步骤标`[✗ SKIP]`并记录原因，继续后续步骤
4. **依赖传播**：跳过的步骤，后续依赖项标[SKIP]

### 第三阶：中断上报（降级仍无法推进时）
1. **触发条件**：关键步骤失败 + 降级方案不可行 + 连续2步失败
2. **动作**：暂停执行态，向用户完整报告：
   - 已完成的步骤和产出
   - 失败步骤的具体错误
   - 唧建议的修复方案（至少2个）
3. **用户决策后**：按用户选择修正plan.md，从失败步骤恢复执行

### plan修正规则
- 回退到规划态修正plan.md，重新过审查门（步骤5-6）
- 修正时只改失败相关部分，保留已完成步骤的[✓]标记
