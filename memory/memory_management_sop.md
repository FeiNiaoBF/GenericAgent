## 执行摘要（≥1步执行前必读）
1. 核验写入：所有L1/L2/L3写入需源自成功工具调用(No Execution, No Memory)
2. 修改前读本META-SOP §validate：反垃圾/反幻觉/反冗余
3. L1感知价值审查 → L2内容压缩 → L4 session蒸馏 → 🛑 过验证门禁
...
5.  **反射模式 (Reflection Mode - 写日记)**
    *   **定义**：每轮任务完成（给用户输出最终总结后），必须在今日 Obsidian 日记 `00.Chronicles/Daily/YYYY-MM-DD.md` 的 `🐣 唧の足迹` 下追加一行。若写入失败，至少告知用户。
    *   **格式**：`- [HH:mm] **<任务名>** — ≤30字简述`
    *   **例外**：测试性小操作、用户明确说"不用记"。
    *   **原理**：维持主人对当日工作的感知闭环，避免任务"做完就忘"。
---
## 记忆层级架构
```
...
---
## Git操作提示
- 用户PC有全局git别名。核心：`a`(add) `cm`(commit -m) `st`(status -s) `co`(checkout) `br`(branch -vv) `ps`(push) `pr`(pull --rebase) `f`(fetch)
- 扩展：`aa`(add --all) `cob`(checkout -b) `last`(log -1 HEAD) `lol`(graph log) `undo`(reset HEAD~1) `amend`(commit --amend --no-edit)
- 通过 `subprocess.run(['git', 'alias', ...])` 调用（勿用os.system或shell字符串）
- Git闭环：改完→测试→git a → git cm "msg" → git ps

## L1整理与压缩（ROI驱动）
### ROI公式
ROI = (不放该词的犯错概率 × 代价) / 每轮词数成本。两类留存: **存在性指针**(场景↔方案触发词，仅反直觉时保留)、**行为规则**(不提醒就会犯的错，ROI过门槛则留)。

### 快速判断
||| 该留: 反直觉触发词(没提示就想不到查SOP的场景)
||| 该删: 翻译/内容描述/实现细节/L3文件名已自解释的冗余

### 压缩四原则
||| ①最小充分指针 ②存在性编码(意识某类知识存在即可取用深层) ③ROI排序(高ROI占一行，低ROI合并) ④结构优于描述(L3文件/目录名自解释)

### 整理流程
1. 逐行读L1，按`|`拆片段，分类: 存在性指针/RULES/翻译/内容描述/实现细节/冗余
2. 先清RULES: 全局高ROI→留; 特定场景/低危险→降级L3或删除
3. 再清存在性指针: 检查场景↔方案存在性; 场景触发词仅反直觉时添加; 翻译/内容/实现细节删除
4. 验证总行数≤30

### 红线
记忆修改是持久性伤害。L1只能**patch词级修改**，禁overwrite。产生误导应及时修正L1或记忆更名。

### 目录污染清理
memory/ 只放L3 SOP/工具。非L3文件分类清理。脚本: `memory_cleanup.py --dry-run`(预览) / `--execute`(执行)：
- 运行时统计/JSON → `temp/`
...
### L1审计快速清单
对比官方模板 `assets/global_mem_insight_template.txt` 查5要素。脚本: `audit_l1.py` (自动检查行数/要素/tier标记)：
...
### 自我进化触发 (Ch13 §13.5)
当以下条件满足时，应主动调用 start_long_term_update 蒸馏长期记忆：
1. 任务持续 15+ 轮 → 必定触发
2. 发现新的避坑/配置/模式 → 信息密度高时触发
3. 用户明确要求"进化/优化/学习" → 立即触发
4. 外部文档(SOP/官方文档)学完并验证后 → 立即蒸馏

进化产物: (a) L1/RULES新增或强化 (b) L2压缩/重组 (c) novo L3 SOP (d) TODO新增自主任务
