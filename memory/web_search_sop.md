...
**"深度查找"停止条件（5 项全满足才算完成）：**
- [ ] **交叉验证**：同一事实 ≥2 个独立来源（不同域名 / 不同作者 / 不互相引用）
- [ ] **详情确认**：≥1 个详情页全文确认关键数据，不靠摘要
- [ ] **权威分层**：官方文档 > 技术博客/论文 > 问答社区(SO/Reddit) > 个人博客
- [ ] **时效检查**：确认发布时间，过时信息降权或标注
- [ ] **矛盾裁决**：来源冲突时优先高权威源；无法裁决则标注"存在争议" + 列出双方论据

**搜索轮次上限（防止无限深挖）：**
- **第 1 轮**：最佳关键词 + site:权威源 → web_scan 提取 → 打开详情页
- **第 1 轮无果** → **第 2 轮**：换关键词/角度/语言（中英文各一次）→ 同样流程
- **第 2 轮仍无果** → 🚫 终止，输出"⚠️ 深度查找未找到可靠答案"，禁止第 3 轮

**独立来源判定（避免伪交叉验证）：**
- ✅ 不同域名 + 不同作者 → 独立
- ✅ 同一域名不同作者 (如 SO/Reddit) → 部分独立，权重减半
- ❌ 同一篇文章被转载 → 不独立，计为 1 个来源
- ❌ A 引用 B → 不独立，需追溯到原始来源 B
...
### Phase 7: 深度查找协议（Deep Search Protocol）

> 对应 L1 RULES 中 `web_search_sop深度查找` 的操作定义。

**定义**：深度查找 ≠ 一次搜索出结果。它是一次搜索不够就知道搜第二遍、打开详情页、交叉验证后才能输出的搜索模式。

**触发条件**（任一满足即触发）：
- VKB 无对应条目（`vault_knowledge_sop §9` 检索空）
- LLM 自带知识不确定或内部冲突
- 需要最新事实（API 变更、新闻事件、安全漏洞）

**标准流程**：
```
Phase 2 构造查询 → Phase 3 执行搜索 → Phase 4 提取 Top 2-3 结果
    → web_scan 打开详情页 → Phase 5 交叉验证
    → 满足停止条件？→ Phase 6 整理引用 → 输出
    → 不满足？→ 第2轮换关键词/语言 → 重复
    → 第2轮仍不满足？→ 终止，标注不确定性
```
...
**与 VKB 入库的衔接**：
- 深度查找成功 → 事实已交叉验证 → 提醒用户：`是否需要记入 VKB？`
- → `vault_knowledge_sop §5` 蒸馏管道 → `05.Knowledge/` 新条目
...
| 无限搜索无停止条件 | 遵守 Phase 5 两轮上限 + Phase 7 深度查找协议，第2轮无果即终止 |
| 搜索完忘记入库 | 深度查找成功 → 提醒用户记入 VKB → vault_knowledge_sop §5 蒸馏管道 |
...
[ ]  3  新标签页打开 + 等待加载
[ ]  4  提取结果（标题+URL+摘要）
[ ]  5  核实：≥2独立来源 + ≥1详情页全文 + 权威分层 + 时效 + 矛盾裁决
[ ]  6  整理引用（来源 URL + 事实/观点）
[ ]  7  深度查找：第1轮不满足→第2轮换词/换语言→仍不满足则终止标注不确定性
```
