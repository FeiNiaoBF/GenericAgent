...
### ⚠️ 次级MOC（学习类/子领域MOC，level:sub）

| | 次级MOC（学习类/子领域） |
|---|---|
| **模板** | `99.System/Templates/MOC.md` (v3.0)，但**精简**：无dataview/无边界区块 |
| **定位** | 从属于某个顶层MOC的学习路径或子领域聚合 |
| **frontmatter** | `level: sub` + `parent: "[[父MOC名]]"` + 无moc字段 |
| **区块** | 标题 → 核心目标 → 学习路径/内容 → 资源 → 下一步 → 关系图（精简结构） |
| **何时建** | 学习类主题（如TS/SE/算法），从属于顶层MOC但本身不是独立领域 |
| **禁手** | 禁用顶层MOC模板(01-05Cat) | 禁填dataview查询 | 禁设边界/增长引擎等顶层专属区块 |

**决策树**: 新学科/领域级→顶层MOC(01-05Cat模板,不填moc) | 领域内某主题→主题MOC(MOC.md v3.0,moc必填) | 学习类/子领域→次级MOC(MOC.md精简版,level:sub)
