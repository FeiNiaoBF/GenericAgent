...
⚠️ 前置规则:
  1. 先枚举窗口确认目标窗口存在且已激活
  2. 🚫 禁止全屏截图: 先用ljqCtrl截窗口区域
  3. 能不用vision就不用: 优先OCR/窗口标题
用法: result = ask_vision(image, prompt="...", backend="claude", timeout=60)
  image: 文件路径(str/Path) 或 PIL Image
  backend: 'claude'(默认) | 'openai' | 'modelscope'
配置在文件末尾，按需修改 API key 和后端选择
"""
...
CLAUDE_CONFIG_KEY = 'native_claude_config_deepseek'   # mykey.py 中 Claude 配置的变量名
OPENAI_CONFIG_KEY = 'native_oai_config'               # mykey.py 中 OpenAI 配置的变量名
...
def _prepare_image(image_input, max_pixels=1440000):
    ...
    if w * h > max_pixels:
        ...
        print(f"  \U0001f4d0 缩放: {w}×{h} → {new_w}×{new_h}")
    ...
    print(f"  \U0001f4e6 Base64: {len(buf.getvalue())/1024:.1f}KB")
    return b64
